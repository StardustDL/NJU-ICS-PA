	...
	for (; n > 0; n--) {
...
#ifdef DEBUG
		{
			...
			for (WP * cur = getHeadWP(); cur != NULL;
			     cur = cur->next) {
				bool success = false;
				uint32_t newVal = expr(cur->expr, &success);
				if (!success) {
					WarningN("WP failed: %s", cur->expr);
					isTrigger = true;
				}
				if (newVal != cur->lastVal) {
					InfoN("WP changed: %s : %u -> %u",
					      cur->expr, cur->lastVal,
					      newVal);
					cur->lastVal = newVal;
					isTrigger = true;
				}
		...
		if (nemu_state != NEMU_RUNNING) {
			if (nemu_state == NEMU_END) {
				...
				printflog
					("\33[1;31mnemu: ABORT\33[0m at eip = 0x%08x\n\n",
					 cpu.eip);
				monitor_statistic();
				return;
			}
