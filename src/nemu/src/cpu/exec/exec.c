...
static inline void idex(vaddr_t *eip, opcode_entry *e) {
  ...
  // Info("Before decode: %x",*eip);

  if (e->decode)
    e->decode(eip);

#ifdef DEBUG
  // Log("%06x %s,%s,%s",*eip,id_src->str,id_src2->str,id_dest->str);
  // Log("%06x %x,%x,%x",*eip,id_src->val,id_src2->val,id_dest->val);
#endif

  e->execute(eip);

  // Info("After execute: %x",*eip);
}
...
make_group(gp2,
  ...
  /* 0xf6, 0xf7 */
make_group(gp3,
    /*Not same as group table*/IDEX(test_I, test), EMPTY, EX(not), EX(neg),
