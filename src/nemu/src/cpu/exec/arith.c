...
make_EHelper(add) {
  ...
  rtl_not(&t0, &t0); // t0 = sign(a)==sign(b)
  rtl_xor(&t1, &id_dest->val, &t2); // t1 = sign(c)!=sign(a)
  rtl_and(&t0, &t0, &t1);
...
make_EHelper(sub) {
  ...
  // a-b=c
  rtl_xor(&t0, &id_dest->val, &id_src->val); // t0 = sign(a)!=sign(b)  
  rtl_xor(&t1, &id_dest->val, &t2); // t1 = sign(c)!=sign(a)
  rtl_and(&t0, &t0, &t1);
  rtl_msb(&t0, &t0, id_dest->width);
  rtl_set_OF(&t0);
...
make_EHelper(cmp) {
  // Warning("cmp %x with %x",id_src->val,id_dest->val);
...
make_EHelper(inc) {
  ...
  rtl_addi(&t2, &id_dest->val, imm);
  rtl_setrelop(RELOP_LTU, &t0, &t2, &id_dest->val);
  operand_write(id_dest, &t2);

  // Warning("inc %x -> %x",id_dest->val, t2);
...
make_EHelper(dec) {
  // FE /1
  // copy from sub, replace src1->val to imm

  const int imm = 1;

  // Error("iddest: %x",id_dest->val);

  rtl_subi(&t2, &id_dest->val, imm);
  rtl_setrelopi(RELOP_LTU, &t0, &id_dest->val, imm);
  operand_write(id_dest, &t2);

  // Warning("dec %x -> %x",id_dest->val, t2);
  ...
  // a-b=c
  rtl_xori(&t0, &id_dest->val, imm); // t0 = sign(a)!=sign(b)  
  rtl_xor(&t1, &id_dest->val, &t2); // t1 = sign(c)!=sign(a)
  rtl_and(&t0, &t0, &t1);
  rtl_msb(&t0, &t0, id_dest->width);
  rtl_set_OF(&t0);
...
make_EHelper(neg) {
  ...
  rtl_xor(&t1, &id_dest->val, &t2); // a^-a
  
  /*rtl_msb(&t1, &t1, id_dest->width); // get sign bit
  rtl_xor(&t0, &t0, &t1); // t0!=t1 => t0=1 && t1=0*/

  rtl_sext(&t1, &t1, id_dest->width);
  rtl_setrelopi(RELOP_EQ, &t1, &t1, 0); // (a^-a) == 0 => a=b1000 or b0000
  rtl_and(&t1, &t1, &t0); // t1 = (a^-a) == 0 && a != 0 => a=b1000
  rtl_set_OF(&t1);
