...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  // static uint32_t cns[2] = {0xffffff00, 0xffff0000};

  // assert(width==1 || width == 2);
  assert(width>0);
  /*rtl_msb(&at,src1,width);
  if(at){
    if(width == 1) *dest = *src1 | cns[0];
    else *dest = *src1 | cns[1];
  }
  else *dest = *src1;*/

  if(width<4){
    // Warning("dest %d",*dest);
    *dest = ((int32_t)(((int32_t)(*src1)) << (32-width*8))) >> (32-width*8);
    // Warning("dest %d",*dest);
  }
  else{
    *dest = *src1;
